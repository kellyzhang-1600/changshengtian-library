...
import { categoryLabel, getLocale, textTitle, textSummary, textTags, textPeriod } from "@/lib/i18n";
  ...
  return (
    <Link href={`/archive/${text.slug}?lang=${lang}`} className="group block border border-archive/12 bg-vellum p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-archive">
      <div className="flex items-center justify-between gap-4 text-xs text-muted">
        ...
        {textPeriod(text, lang) ? <span>{textPeriod(text, lang)}</span> : null}
      </div>
      <h3 className="mt-4 font-serif text-2xl text-archive">{textTitle(text, lang)}</h3>
      <p className="mt-3 line-clamp-3 text-sm leading-7 text-muted">{textSummary(text, lang)}</p>
      <div className="mt-5 flex flex-wrap gap-2">
        {textTags(text, lang).slice(0, 4).map((tag) => (
